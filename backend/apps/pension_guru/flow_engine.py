...
class PensionFlow:
    def __init__(self, profile: Optional[dict[str, Any]], user_id: str):
        ...
        self.current_step_name: str = self._get_profile_value(
            "pending_step", "step_1_welcome_intro"
        )
        logger.debug(
        ...
        try:
            ...
            if "branching_logic" in current_node:
                ...
                if region:
                    ...
                    if not target_flow:
                        ...
                        if (
                            "condition" not in fallback
                            or "other" in fallback.get("condition", "").lower()
                        ):
                ...
                if target_flow:
                    self.memory.save_user_profile(
                        self.user_id, {"pending_step": target_flow}
                    )
            ...
            if expected_field:
                if self._get_profile_value(expected_field) is not None:
                    self.memory.save_user_profile(
                        self.user_id, {"pending_step": next_step_name}
                    )
                    self.profile = self.memory.get_user_profile(self.user_id)
                    if next_step_name:
