...
def test_whitesands_facts_live_scrape():
    ...
    assert any(keyword in facts for keyword in ["check-in", "amenities", "contact"]), "Expected key terms in facts"
...
def test_full_whitesands_scrape_raw_content():
    ...
    assert len(matches) >= 2, f"Expected at least 2 keywords in raw content, found: {matches}"
