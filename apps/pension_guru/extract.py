...
def wants_tips(profile, msg: str, history: list) -> bool:
...
def should_offer_tips(reply: str) -> bool:
    reply = reply.lower()
    return any(
...
def render_profile_field(field, profile):
    if field == "income":
...
def extract_user_data(user_id: str, msg: str):
    ...
    if region == "unsupported":
        ...
        save_chat_message(user_id, 'assistant', block_msg)
        return False  # Don't update profile
...
def handle_empty_input(user_id, history, profile, tone):
    import re
    from gpt_engine import get_gpt_response, config, save_chat_message, save_user_profile
    ...
    if history and len(history) >= 2:
        if "how many years of prsi contributions" in history[-2]["content"].lower():
            if profile and profile.region in ["Ireland", "UK"] and getattr(profile, "prsi_years", None):
                ...
                reply = (
                    f"For {calc['prsi_years'] if 'prsi_years' in calc else calc['ni_years']} years of contributions in {calc['region']}:\n"
